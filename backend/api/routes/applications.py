...
def _serialize_application(doc) -> dict:
    """Helper: converts a Firestore application document to a JSON-serializable dict."""
    d = doc.to_dict()
...
async def get_recruiter_applications(user: dict = Depends(get_current_user)):
    """Returns all applications across all jobs posted by the authenticated recruiter,
    sorted by most recent first."""
    db = firebase_db()
...
async def get_applications_for_job(job_id: str, user: dict = Depends(get_current_user)):
    ...
    Only the recruiter who owns the job should call this endpoint."""
    db = firebase_db()
...
async def update_application_status(
    ...
    """Lets a recruiter update the status of an application (e.g. 'Shortlisted', 'Rejected').
    Returns HTTP 403 if the calling user is not the recruiter for this application."""
    db = firebase_db()
