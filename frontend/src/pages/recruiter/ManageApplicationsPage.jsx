...
export function ManageApplicationsPage() {
  ...
  const isLocumApplication = (app) => {
    const jobType = String(app?.jobType || '').toLowerCase()
    if (jobType.includes('locum')) return true
    const job = jobs.find(j => String(j.id) === String(app?.jobId))
    return isLocumJob(job)
  }

  const visibleJobs = isLocumMode ? jobs.filter(isLocumJob) : jobs.filter(j => !isLocumJob(j))
  ...
  const getStatusBreakdown = (jobId, jobTitle) => {
    const apps = visibleApplications.filter(a => String(a.jobId) === String(jobId) || a.jobTitle === jobTitle)
    return {
    ...
    <div className="max-w-[900px] mx-auto px-6 py-8 pb-20 font-sans">
      ...
      {visibleJobs.length === 0 ? (
        <div className="bg-white border border-border rounded-xl p-10 text-center">
          ...
          <div className="text-sm font-semibold text-gray-700 mb-1">No {isLocumMode ? 'locum posts' : 'job posts'} yet</div>
          <div className="text-xs text-gray-400 mb-4">Post a {isLocumMode ? 'locum shift' : 'job'} to start receiving applications.</div>
          <Link to={postPath} className="px-4 py-2 bg-blue-600 text-white text-xs font-semibold rounded-lg">Post a {isLocumMode ? 'locum shift' : 'job'}</Link>
        </div>
      ) : (
        <div className="space-y-3">
          {visibleJobs.map(job => {
            ...
            return (
              <div
                key={job.id}
                onClick={() => navigate(`${applicationsPath}/${job.id}`)}
