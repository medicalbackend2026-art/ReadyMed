...
export function CompanySetupPage() {
  ...
  return (
    <div className="max-w-[1100px] mx-auto px-6 py-10 pb-20 font-sans">
      ...
      <BackButton onClick={() => navigate('/recruiter/services')} label="Back" />
                ...
                <nav className="space-y-1">
                  {sections.map((sec, idx) => (
                    <button
                      ...
                      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left transition-colors text-sm ${activeSection === idx ? 'bg-blue-50 text-blue-700 font-semibold' : 'text-gray-700 hover:bg-gray-50'
                        }`}
                    >
                      {liveComplete[idx] ? (
                ...
                <div className="flex gap-2 overflow-x-auto pb-2">
                  {sections.map((sec, idx) => (
                    <button key={idx} type="button" onClick={() => setActiveSection(idx)}
                      className={`flex items-center gap-1.5 px-3 py-2 rounded-full border text-xs font-medium whitespace-nowrap shrink-0 transition-colors ${activeSection === idx ? 'border-blue-600 bg-blue-50 text-blue-700'
                              ...
                              <div className="grid grid-cols-2 gap-2.5">
                                {types.map(t => (
                                  <button key={t.name} type="button" onClick={() => setOrgType(t.name)}
                                    className={`p-3.5 border-2 rounded-[10px] text-center transition-all text-[13px] font-medium ${orgType === t.name ? 'border-blue-600 bg-blue-50 text-blue-700' : 'border-border text-gray-700 hover:border-blue-200 hover:bg-blue-50'
                                      }`}>
